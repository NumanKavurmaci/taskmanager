...
def index():
    ...
    # Generate the system-generated notification content
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    # Query the tasks for upcoming due dates
    upcoming_tasks = session.query(Task).filter(Task.due_date > current_time).all()

    # Create notification objects for each upcoming task
    new_notifications = []
    for task in upcoming_tasks:
        ...
        time_remaining = task.due_date - datetime.now().date()
...
def create_team():
    if request.method == 'POST':
        ...
        team = Team(name=team_name, description=description, users=team_members)
